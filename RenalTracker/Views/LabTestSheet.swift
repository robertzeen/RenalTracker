...
struct LabTestSheet: View {
    ...
    init(userProfile: UserProfile?) {
        ...
        if let date = userProfile?.nextLabTest {
                ...
                VStack(spacing: 12) {
                    ...
                    HStack(spacing: 12) {
                        ...
                        VStack(alignment: .leading, spacing: 2) {
                            ...
                            TextField("Лаборатория (опционально)", text: $labLocation)
    ...
    private func save() {
        userProfile?.nextLabTest = selectedDate
        try? modelContext.save()
        NotificationManager.shared.scheduleLabTestNotification(date: selectedDate)
    }

    private func addToCalendar(completion: @escaping () -> Void) {
        let store = EKEventStore()
        store.requestFullAccessToEvents { granted, _ in
            guard granted else { return }
            DispatchQueue.main.async {
                let event = EKEvent(eventStore: store)
                event.title = "Сдача анализов"
