...
- Показатели: АД, пульс, вес — графики, история, PDF экспорт
- Анализы: каталог (15 тестов) + кастомные, графики, PDF экспорт
- Лекарства: расписание, отметка приёма, свайп-действия, PDF экспорт
- Визиты к врачу: история, детали, добавление/редактирование
- Настройки: профиль, статус лечения, уведомления
...
- **CustomMetrics UI** — модели и каталог созданы, интерфейс не реализован:
  - Шаги 2–6: добавление кастомных метрик в IndicatorsView (графики, ввод значений, история)
  - SettingsView: управление активными кастомными метриками (включить/выключить из каталога)
...
| `IndicatorsView.swift` | Графики АД/пульса/веса, фильтры 7/30/все; `ChartPeriod` picker |
...
| `SettingsView.swift` | Личные данные, статус лечения, переключатели уведомлений с пикерами времени |
